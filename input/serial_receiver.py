    ...
    def read_serial(self):
        while True:
            ...
            payload = Payload(
                ...
                altitude=float(line.split(",")[6]) if line.split(",")[6] else 0.0,
                temperature=float(line.split(",")[7]) if line.split(",")[7] else 0.0,
                sats=int(line.split(",")[8]) if line.split(",")[8] else 0,
                battery=float(line.split(",")[9]) if line.split(",")[9] else 0.0,
                pressure=float(line.split(",")[10]) if line.split(",")[10] else 0.0,
                speed=float(line.split(",")[11]) if line.split(",")[11] else 0.0,
                ascent_rate=None,
                other_fields={
